...
namespace MTGProxyMaker
{
    public class MtgController
    {
        ...
        private static HtmlDocument DownloadCardWebPage(string cardName)
        {
            try
            {
                return DownloadWebPage(String.Format("http://gatherer.wizards.com/Pages/Search/Default.aspx?name={0}", cardName));
            }
            catch (Exception)
            {
            }
        ...
        private static Card ScrapeCardDetails(HtmlDocument hd)
        {
            ...
            try
            {
                ...
                {
                    //image url
                    var url1 = hd.DocumentNode.SelectSingleNode("//td[@class='leftCol']/img").Attributes["src"].Value;
                ...
                {
                    //flavour
                    c.FlavourText = Clean(h.SelectNodes("//div[contains(@id,'FlavorText')]/div[@class='cardtextbox']/i").Select(s => s.InnerHtml).Aggregate((a, b) => a + "," + b));
                }
                catch { }
